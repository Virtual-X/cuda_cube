...
//		return;
	...
	for (int candidate = actualPiece; candidate < PiecesCount; candidate++) {
		...
		for (int j = 0; j < totCount; j += BlockSize) {
			...
			if (idx < nvalid) {
				if (Debug && CheckIndex(__id2[idx] + (PiecesCount - 1) * MaxOutput, PiecesCount * PiecesCount * MaxOutput, globals, ErrorCodeInvalidId2))
					return;

				const int* current = locals.d_situation + __id2[idx]; // somewhat increasing

				const int other_ind = _temp[BlockSize] + idx; // coalesced!

				if (Debug && CheckIndex(other_ind, MaxOutput, globals, ErrorCodeInvalidId2))
					return;

				const int maskIndex = __index[idx];

				if (IsLastStep) {
					AddSolution<Debug>(globals, actualPiece, current, other_ind, maskIndex);
				}
				else {
					if (!InitNextStep<Debug>(globals, locals, actualPiece, current, other_ind, maskIndex, candidate, __pos[idx], __grid[idx]))
						return;
				}
			}
		}
