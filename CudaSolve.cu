...
void GetBeginAndCount(const Globals globals, const int* situation, const int2* candidatesOffsets, const int candidate,
	...
	begin = beginEnd.x;
	count = beginEnd.y - beginEnd.x;
}

template<bool Debug>
__device__
void GetBeginAndCount_(const Globals globals, const int* situation, const int2* candidatesOffsets, const int candidate,
	...
	const int idx = threadIdx.x;
	GetBeginAndCount<Debug>(globals, situation, candidatesOffsets, candidate, _begin[idx], _count[idx]);
}
...
void CudaStep(const Globals globals, const Locals locals, const int actualPiece, const int off, const int count) {
	...
	__shared__ vint _candidate[BlockSize];
	__shared__ vint _px[BlockSize];
	__shared__ vint _count;
	...
	if (idx < blockCount) {
		for (int candidate = actualPiece; candidate < PiecesCount; candidate++) {
			GetBeginAndCount<Debug>(globals, situation, candidatesOffsets, candidate, myb[candidate], myc[candidate]);
			mytot += myc[candidate];
		}
	}
	...
	const int myoff = atomicAdd((int*)&_count, mytot);

	__syncthreads();

	{
		const int totCount = _count;
		...
		for (int j = 0; j < totCount; j += BlockSize) {
			...
			if (idx < blockCount) {
				mytot = 0;
				for (int candidate = actualPiece; candidate < PiecesCount; candidate++) {
					const int b = max(myoff - j + mytot, 0);
					const int e = min(myoff - j + mytot + myc[candidate], BlockSize);
					for (int i = b; i < e; i++) {
						_px[i] = idx;
						_begin[i] = myb[candidate] + i - b;
						_candidate[i] = candidate;
					}
					mytot += myc[candidate];
				}
			}
...
//			__syncthreads(); // for compacting
			...
			if (valid) {
				...
				const int other_ind = _count + target;
				...
				if (IsLastStep) {
					...
					if (!InitNextStep<Debug>(globals, locals, actualPiece, current, other_ind, maskIndex, _candidate[idx], _pos[px], mask | grid))
						return;
				}
			}
//			__syncthreads();
